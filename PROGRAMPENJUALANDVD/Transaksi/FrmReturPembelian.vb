...
Public Class FrmReturPembelian
    ...
    Sub GenerateNoRetur()
        ...
        OpenConnection()
        Dim query As String = " Select noreturbarang from tbretur where noreturbarang in " &
                              " ( Select max (noreturbarang) as noreturbarang from tbretur)"
        cmd = New SqlCommand(query, conn)
        dr = cmd.ExecuteReader
        dr.Read()

        If Not dr.HasRows Then
            urutan = "RT/" & DateTime.Now.Year & "/" & "000001"
        Else
            ...
            If Microsoft.VisualBasic.Left(dr.GetString(0), 8) <> "NO/" & DateTime.Now.Year & "/" Then
                urutan = "RT/" & DateTime.Now.Year & "/" & 1
            Else
                hitung = Microsoft.VisualBasic.Right(dr.GetString(0), 6) + 1
                urutan = "RT/" & DateTime.Now.Year & "/" & Microsoft.VisualBasic.Right("000000" & hitung, 6)
            End If
        End If
        dr.Close()
        txtkodepembelian.Text = urutan
    End Sub
    Sub HapusTextItem()
        txtkodeitem.Clear()
        txtnamaitem.Clear()
    ...
    Sub RetrieveMasterItem()
        ...
        dr.Close()
    End Sub
    Sub RetrieveNoReturHeader()
        Dim query As String = "Select RT.noreturbarang,RT.tanggalretur,RT.Status"
        query += " From tbretur as RT"
        query += " Where RT.noreturbarang = '" & noRT & "'"
        Try
            OpenConnection()
            cmd = New SqlCommand(query, conn)
            dr = cmd.ExecuteReader
            dr.Read()
            If dr.HasRows Then
                txtkodepembelian.Text = dr.Item("noreturbarang")
                noRT = dr.Item("noreturbarang")
                tglpembelian.Value = dr.Item("tanggalretur")
        ...
        End Try
    End Sub
    Sub RetrievenoreturbarangDetail()
        Dim query As String = "Select rtdet.*"
        query += ", item.namaitem, item.jenisitem"
        query += " From tbreturdetail as rtdet"
        query += " inner join tbmasteritem as item on item.kodeitem = rtdet.kodeitem"
        query += " Where rtdet.noreturbarang = '" & noRT & "'"
        Try
            OpenConnection()
            cmd = New SqlCommand(query, conn)
            dr = cmd.ExecuteReader
            TampilGridDetail()
            While dr.Read
                If Not IsDBNull(dr.Item("noreturbarang")) Then
                    dgv.Rows.Add()
    ...
    Private Function CreateData() As Boolean
        ...
        Try
            ...
            SQLHeader = "Insert into tbretur(noreturbarang,tanggalretur,status,kodepegawai)values" &
                        "('" & txtkodepembelian.Text & "'" &
                        ",'" & Format(Me.tglpembelian.Value, "yyyy-MM-dd") & "'" &
                        ",1,'" & MenuUtama.stKodePegawai.Text & "')"
            cmd = New SqlCommand(SQLHeader, conn, Trans)
            cmd.ExecuteNonQuery()

            For detail = 0 To Me.dgv.Rows.Count - 2
                SQLDetail = "Insert into tbreturdetail(noreturbarang,kodeitem,qty)values" &
                            "('" & txtkodepembelian.Text & "'" &
    ...
    Private Function UpdateData() As Boolean
        ...
        Trans = conn.BeginTransaction
        Try
            SQLHeader = "Update tbretur Set kodeitem='" & txtkodeitem.Text & "' Where noreturbarang='" & txtkodepembelian.Text & "'"

            cmd = New SqlCommand(SQLHeader, conn, Trans)
            cmd.ExecuteNonQuery()

            Dim deleteSql As String = "Delete From tbbarangmasudetail Where noreturbarang='" & txtkodepembelian.Text & "'"
            cmd = New SqlCommand(deleteSql, conn, Trans)
            cmd.ExecuteNonQuery()

            For detail = 0 To Me.dgv.Rows.Count - 2
                SQLDetail = "Insert into tbbarangmasudetail(noreturbarang,kodeitem,qty)values" &
                            "('" & txtkodepembelian.Text & "'" &
                            ",'" & Me.dgv.Rows(detail).Cells(0).Value & "'" &
                            ",'" & Me.dgv.Rows(detail).Cells(3).Value & "')"
                cmd = New SqlCommand(SQLDetail, conn, Trans)
    ...
    Sub TampilanUpdate()
        Try
            RetrieveNoReturHeader()
            RetrievenoreturbarangDetail()
            If statusno = 1 Then
    ...
    Sub SetujuiData()
        Dim sql As String
        sql = "Update tbretur set status = 3 where noreturbarang = '" & txtkodepembelian.Text & "'"
        Try
            OpenConnection()
    ...
    Sub TolakData()
        Dim sql As String
        sql = "Update tbretur set status = 0 where noreturbarang = '" & txtkodepembelian.Text & "'"
        Try
            OpenConnection()
            cmd = New SqlCommand(sql, conn)
            cmd.ExecuteNonQuery()
